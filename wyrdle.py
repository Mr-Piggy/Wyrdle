...
console = Console(width=40, theme=Theme({"warning": "bold yellow on red"}))

WORDLENGTH = 5
NUMBEROFGUESSES = 6
WORDSPATH = pathlib.Path(__file__).parent / "words"

def main():
    # Pre-process
    word = get_random_word(WORDSPATH.read_text(encoding="utf-8").split("\n"))
    guesses = ["_" * WORDLENGTH] * NUMBEROFGUESSES

    # Process (main loop)
    with contextlib.suppress(KeyboardInterrupt):
        for idx in range(NUMBEROFGUESSES):
...
def guess_word(previous_guesses):
    ...
    if len(guess) != WORDLENGTH:
        console.print(f"Your guess must be {WORDLENGTH} letters.", style="warning")
        return guess_word(previous_guesses)
